...
If NOT IsNull(registryValues) Then
	If UBound(registryValues) >= 0 And Err.Number = 0 Then
		...
		For Each serviceName in registryValues
			...
			' We want to check for multiple services, in case of a wildcard in servicename
			wmiQuery = "Select * from Win32_Service where StartMode = 'Auto' AND Name LIKE '" & Replace(serviceName,"*","%") & "'"
			Set wmiObject = GetObject("winmgmts:{impersonationLevel=impersonate}!\\" & computerName & "\root\cimv2")
			Set wmiServiceObjects = wmiObject.ExecQuery(wmiQuery)

			If NOT IsNull(wmiServiceObjects) Then
				For Each wmiServiceObject in wmiServiceObjects
					Set serviceObject = New Service
					returnValue = serviceObject.GetServiceInfo(wmiServiceObject.Name,computerName)
...
Class Service
	...
	Public Function GetServiceInfo(mServiceName, mComputerName)
		...
		Set wmiServices = wmi.ExecQuery("Select * from Win32_Service where Name like '" & mServiceName & "'")
